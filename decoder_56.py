...
input_data = [0,1,14,0,35,18,9,49,54,26,32,29,44,40,14,9,53,8,21,34,12,24,21,18,21,3,9,21,43,3,4,39,23]
...
for i, code in enumerate(input_data):
    ...
    if code in INITIAL_LIST: # 초성 
        ...
        if next_code in FINAL_LIST or prev_code in FINAL_LIST: #뒤에 종성이 올 경우
        ...
        while input_data[i] != 0:
            ...
            i += 1
#약어
    elif (code == 1) and (input_data[prev_code] == 0) and (input_data[next_code] in AND_LIST) and (input_data[i+2] == 0):
        i += 1
        result.append(AND_LIST[next_code])
