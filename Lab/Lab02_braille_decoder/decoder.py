...
def sort_data(xyxy):
    ...
    for i in index:
        ...
        line['x'] = xyxy_rev['x'][0:i+1]
        line['xmax'] = xyxy_rev['xmax'][0:i+1]
        line['xmin'] = xyxy_rev['xmin'][0:i+1]
        line['name'] = xyxy_rev['name'][0:i+1]
        ...
        for i,x in enumerate(line_sort['x']):
            if( i != len(line_sort['x'])-1):
                if(line_sort['x'][i+1] - line_sort['x'][i] >= (x_mean * 0.1)):
                    if x >= x_limt:
                        data_lst.append(0)
...
if __name__ == "__main__":
    input_img = './data/images/KakaoTalk_20240316_004226308_03_jpg.rf.688c4bc451d5de22f5d002681a23a0f2.jpg'
    xyxy = yolo_detect(input_img)
    print(xyxy)
    data_lst = sort_data(xyxy)
    print(data_lst)
    txt = trans_data(data_lst)
    print('----------------------------------------')
    print('result_Before:',txt)
